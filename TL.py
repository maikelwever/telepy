...
class TL:
    ...
    def serialize_obj(self, type_, **kwargs):
        bytes_io = io.BytesIO()
        try:
            tl_constructor = self.constructor_type[type_]
        except KeyError:
    ...
    def serialize_method(self, type_, **kwargs):
        bytes_io = io.BytesIO()
        try:
            tl_method = self.method_name[type_]
        except KeyError:
            raise Exception("Could not extract type: %s" % type_)
    ...
    def serialize_param(self, bytes_io, type_, value):
        if type_ == "int":
    ...
    def deserialize(self, bytes_io, type_=None, subtype=None):
        ...
        if type_ == 'int':
            ...
            try:
                # Bare types
                tl_elem = self.constructor_type[type_]
            except KeyError:
                # Boxed types
...
def serialize_obj(type_, **kwargs):
    return tl.serialize_obj(type_, **kwargs)


def serialize_method(type_, **kwargs):
    return tl.serialize_method(type_, **kwargs)


def serialize_param(bytes_io, type_, value):
    return tl.serialize_param(bytes_io, type_, value)
...
def deserialize(bytes_io, type_=None, subtype=None):
    ...
    :type bytes_io: io.BytesIO object
    """
    return tl.deserialize(bytes_io, type_=type_, subtype=subtype)
